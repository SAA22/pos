...
class controllerProducts{
	...
	static public function ctrCreateProducts(){
		...
		if(isset($_POST["newDescription"])){
			...
			if(preg_match('/^[a-zA-Z0-9-ȘȚÎĂÂșțîăâñÑáéíóúÁÉÍÓÚ,& ]+$/', $_POST["newDescription"]) &&
			   ...
			   preg_match('/^[0-9.]+$/', $_POST["newSellingPrice"])){
				...
				if($answer == "ok"){
					...
					echo'<script>
						...
						swal({
							  type: "success",
							  title: "Produsul a fost adaugat cu success",
				...
				echo'<script>
					...
					swal({
						  type: "error",
						  title: "Produsul nu poate sa contina caractere speciale,sau campuri libere",
						  showConfirmButton: true,
						  confirmButtonText: "Inchide"
						  }).then(function(result){
							if (result.value) {
	...
	static public function ctrEditProduct(){
		...
		if(isset($_POST["editDescription"])){
			...
			if(preg_match('/^[a-zA-Z0-9-ȘȚÎĂÂșțîăâñÑáéíóúÁÉÍÓÚ,& ]+$/', $_POST["editDescription"]) &&
			   ...
			   preg_match('/^[0-9.]+$/', $_POST["editSellingPrice"])){
				...
				if($answer == "ok"){
					...
					echo'<script>
						...
						swal({
							  type: "success",
							  title: "Produsul a fost modificat cu success",
							  showConfirmButton: true,
							  confirmButtonText: "Inchide"
							  }).then(function(result){
										if (result.value) {
	...
	static public function ctrDeleteProduct(){
		...
		if(isset($_GET["idProduct"])){
			...
			if($answer == "ok"){
				...
				swal({
					  type: "success",
					  title: "Produsul a fost sters cu success",
					  showConfirmButton: true,
					  confirmButtonText: "Inchide"
					  }).then(function(result){
								if (result.value) {
