			...
			if (preg_match('/^[a-zA-Z0-9]+$/', $_POST["loginUser"]) && 
				...
				if($answer["user"] == $_POST["loginUser"] && $answer["password"] == $encryptpass){
					...
					if($answer["status"] == 1){
						...
						echo '<br><div class="alert alert-danger">Utilizatorul este dezactivat</div>';
					
					}

				}else{

					echo '<br><div class="alert alert-danger">Utilizatorul sau parola sunt incorecte</div>';
		...
		if (isset($_POST["newUser"])) {
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["newName"]) &&
				...
				if ($answer == 'ok') {
						...
						swal({
							type: "success",
							title: "Utilizator adaugat cu success!",
				...
				echo '<script>
					...
					swal({
						type: "error",
						title: "Fara caractere speciale sau campuri goale",
						showConfirmButton: true,
						confirmButtonText: "Inchide"
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["EditName"])){
				...
				if($_POST["EditPasswd"] != ""){
					...
					if(preg_match('/^[a-zA-Z0-9]+$/', $_POST["EditPasswd"])){
						...
						echo '<script>
							...
							swal({
								type: "error",
								title: "Fara caractere speciale in parola sau campuri libere",
								showConfirmButton: true,
								confirmButtonText: "Inchide"
				...
				if ($answer == 'ok') {
					...
					echo '<script>
						...
						swal({
							type: "success",
							title: "Utilizator modificat cu success",
							showConfirmButton: true,
							confirmButtonText: "Inchide"
					...
					echo '<script>
						...
						swal({
							type: "error",
							title: "Fara caractere speciale in nume sau campuri goale",
							showConfirmButton: true,
							confirmButtonText: "Inchide"
							 }).then(function(result){
	...
	static public function ctrDeleteUser(){
		...
		if(isset($_GET["userId"])){
			...
			if($answer == "ok"){
				...
				swal({
					  type: "success",
					  title: "Utilizatorul a fost sters cu success",
					  showConfirmButton: true,
					  confirmButtonText: "inchide"
