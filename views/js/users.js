...
$(".newPics").change(function(){
	...
	if (newImage["type"] != "image/jpeg" && newImage["type"] != "image/png"){
		...
		swal({
			...
			text: "Imaginea trebuie sa fie JPEG or PNG!",
		...
		swal({
			type: "error",
			title: "Eroare la incarcarea imaginii",
			text: "Imaginea este prea mare. Trebuie sa aiba mai putin de 2Mb!",
			showConfirmButton: true,
			confirmButtonText: "Inchide"
		});
 	...
 	datum.append("activateId", userId);
      ...
      success: function(answer){
      	...
      	if(window.matchMedia("(max-width:767px)").matches){
			...
			swal({
				title: "Utilizatorul a fost modificat",
				type: "success",
				confirmButtonText: "Inchide"	
 	...
 	data.append("validateUser", user);
      ...
      success: function(answer){ 
      	...
      	if(answer){
      		...
      		$("#newUser").parent().after('<div class="alert alert-warning">Acest nume este deja folosit</div>');
...
$(document).on("click", ".btnDeleteUser", function(){
	...
	swal({
		...
		text: "Daca nu esti sigur, poti renunta!",
		type: 'warning',
		showCancelButton: true,
		confirmButtonColor: '#3085d6',
		  cancelButtonColor: '#d33',
		  cancelButtonText: 'Inchide',
		  confirmButtonText: 'Da, sterge utilizator!'
		}).then(function(result){
